...
## import dataset
dataset = pd.read_csv("data/tcga/new_TCGA_BRCA_TNBC_TMM_normalized_CPM.csv", index_col="id")
genes = pd.read_excel("data/gene_ids.xlsx", "217")
pa = "training_results/output_v29/" # train with (qn)
cohort = "tcga"

no_truth = True
if no_truth == False:
    truth = pd.read_excel("data/{0}/{0}_clustered_new.xlsx".format(cohort), "consensus", index_col="id", engine="openpyxl")["kmeans"] # tcga
...
for sets in [1,2,3,4,5]:
    ...
    for random_state in [1,2,3,4,5]:
        trained_model = pickle.load(open(pa+"s{0}/rs{1}/rs{1}_trained_models.pkl".format(sets, random_state), "rb"))
